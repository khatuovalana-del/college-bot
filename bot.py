...
def faculty_kb():
    ...
    builder.add(InlineKeyboardButton(text="🩺 Медицинский", callback_data="fac:Медицинский"))
    builder.adjust(1)
...
def format_pair(pair_tuple, week_type):
    ...
    if week_type == "числитель":
        ...
        return f"{pair_tuple[0]} / {pair_tuple[1]}"
...
async def show_day(call: types.CallbackQuery):
    _, faculty, day_index = call.data.split(":")
    text = format_day(faculty, int(day_index))
